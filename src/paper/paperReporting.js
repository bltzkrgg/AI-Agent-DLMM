...
export function formatPaperPositionsTelegram(positions = [], {
  ...
  const lines = [
    ...
    `Accounting: <code>PRICE-ONLY</code> | Fees: <code>N/A</code>`,
  ];
  ...
  lines.push('', '<i>Fee paper tidak dihitung agar laporan tidak memberi angka palsu.</i>');
  return lines.join('\n');
}
...
export function formatPaperClosedNotification({
  ...
  return [
    ...
    `<i>Price-only simulation; fee income is not estimated.</i>`,
  ].join('\n');
}
