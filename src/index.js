...
bot.onText(/\/setconfig(?:\s+(\S+))?(?:\s+(.+))?/, (msg, match) => {
  ...
  if (!key) {
    const lines = [
      ...
      `gmgnMinAgeHours          = ${cfg.gmgnMinAgeHours}`,
      `gmgnMaxAgeHours          = ${cfg.gmgnMaxAgeHours}`,
      `gmgnRequireKnownAge      = ${cfg.gmgnRequireKnownAge}`,
...
function formatRadarReportTelegram(snapshot) {
  ...
  return [
    ...
    `<code>Rules:</code> age=${Number.isFinite(Number(pf.minAgeHours)) ? Number(pf.minAgeHours) : (Number(radarLive.minPoolAgeHours) || 0)}-${(Number.isFinite(Number(pf.maxAgeHours)) && Number(pf.maxAgeHours) > 0) ? Number(pf.maxAgeHours) : (Number(radarLive.maxPoolAgeHours) || 2160)}h | mcap>=${safeNum(pf.minMcap).toLocaleString()} | vol24h>=${safeNum(pf.minVolume24h).toLocaleString()}`,
