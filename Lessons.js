  ...
  try {
    ...
    try {
      const res = await fetchWithTimeout(
        `https://dlmm-api.meteora.ag/position/list?pool=${poolAddress}&limit=20&sort_key=fee_claimed&order_by=desc`,
        {}, 8000
      );
...
Jangan ada teks lain selain JSON.`;
  ...
  const newLessons = safeParseAI(clean);
...
Respond HANYA dengan JSON array index (0-based) yang cross-pool. Contoh: [0, 2, 5]`;
    ...
    try {
      ...
      const indices = safeParseAI(res.content[0]?.text || '', []);
      indices.forEach(i => {
        if (allLessons[i]) {
