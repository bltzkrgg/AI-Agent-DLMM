...
**Claim fees when:**
- Unclaimed fees >= 3% of deployed capital (claim to lock in gains)
- Unclaimed fees >= 5% of deployed capital (claim immediately — don't wait)
- Do NOT claim if unclaimed fees < 3% of deployed capital — let fees accumulate first
...
2. Claim fees on positions where unclaimed >= 3% of deployed capital
...
- **fee_compounding**: when unclaimed fees >= 3% of deployed AND in range → claim_fees → add_liquidity back to same position
...
- `node cli.js pool-detail --pool <addr>` — is volume still present? fee/TVL still good?
- `node cli.js active-bin --pool <addr>` — how far OOR are we? Edge or completely blown through?
- `node cli.js token-info --query <mint>` — price trend, net buyers, narrative still alive?
