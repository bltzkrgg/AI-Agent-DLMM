...
- LP-style Supertrend entry must also stay near the initial reclaim window: if three or more consecutive closed M15 candles are already above the Supertrend line, treat the setup as late/cooling and HOLD for a fresher reclaim instead of deploying into a dry follow-through.
...
- 2026-06-23: Completed 5.4 late-reclaim hardening in `entryCandleSanity`, `hunterAlpha`, and `pendingDeployQueue`: LP scout/final deploy now reject reclaim setups that are already three closed M15 candles above the Supertrend line, marking them as late/cooling and waiting for a fresher reclaim window instead of entering already-dry momentum.
