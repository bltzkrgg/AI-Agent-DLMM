...
- 2026-06-20: Completed 5.4 + 5.4 mini Pool Alerts mode in `src/index.js`, `src/config.js`, and runtime-state wiring: the bot now supports a discovery-only `Pool Alerts` scheduler with default configurable interval `5m`, Telegram `/poolalerts on|off`, startup/activation/status visibility, and inline buttons. This mode reuses existing Meteora discovery feeds, persists first-seen pool dedupe in `runtime-state.json`, and is mutually exclusive with autoscreen so alert-only awareness never collides with screening/deploy/monitor behavior.
- 2026-06-22: Hardened Pool Alerts source in `src/index.js` + `src/solana/meteora.js`: discovery-only alerts no longer read the ranked hybrid pool feed (`fee_tvl_ratio` / `tvl` / `volume`), and now pull newest Meteora pools first via `sort_by=pool_created_at:desc` with `created_at:desc` fallback. This makes `Pool Alerts` closer to actual new-pool awareness instead of “new pools that later become relevant in ranking.”
